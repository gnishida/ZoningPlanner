...
void MCMC3::setPreferences(std::vector<std::vector<float> >& preference) {
	this->preferences = preference;
}

void MCMC3::addPreference(std::vector<float>& preference) {
	this->preferences.push_back(preference);
}

void MCMC3::setPreferenceForLandValue(std::vector<float>& preference_for_land_value) {
 ...
 */
void MCMC3::findBestPlan(int** zones, int* city_size, std::vector<float>& zoneTypeDistribution, int start_size, int num_layers, std::vector<std::pair<Polygon2D, ZoneType> >& init_zones) {
	srand(10);
...
void MCMC3::computeDistanceMap(int city_size, int* zones, int** dist) {
...
void MCMC3::saveZoneImage(int city_size, int* zones, char* filename) {
...
void MCMC3::loadZone(int city_size, int* zones, char* filename) {
...
void MCMC3::saveZone(int city_size, int* zones, char* filename) {
...
void MCMC3::dumpZone(int city_size, int* zones) {
	printf("<<< Zone Map >>>\n");
	for (int r = 0; r < city_size; ++r) {
		for (int c = 0; c < city_size; ++c) {
...
void MCMC3::dumpDist(int city_size, int* dist, int featureId) {
...
float MCMC3::distToFeature(float dist) {
	return exp(-K * dist);
}

std::vector<float> MCMC3::distToFeature(std::vector<float>& dist) {
...
float MCMC3::featureToDist(float feature) {
	return -logf(feature) / K;
}

std::vector<float> MCMC3::featureToDist(std::vector<float>& feature) {
...
float MCMC3::dot(std::vector<float> v1, std::vector<float> v2) {
	float ret = 0.0f;
...
float MCMC3::randf(float a, float b) {
	return randf() * (b - a) + a;
}

int MCMC3::sampleFromCdf(float* cdf, int num) {
...
int MCMC3::sampleFromPdf(float* pdf, int num) {
	if (num == 0) return 0;
...
inline bool MCMC3::isOcc(int* obst, int s, int featureId) {
	return obst[s * NUM_FEATURES + featureId] == s;
}

inline int MCMC3::distance(int city_size, int pos1, int pos2) {
...
void MCMC3::clearCell(int* dist, int* obst, int s, int featureId) {
	dist[s * NUM_FEATURES + featureId] = MAX_DIST;
	obst[s * NUM_FEATURES + featureId] = BF_CLEARED;
}

void MCMC3::raise(int city_size, std::list<std::pair<int, int> >& queue, int* dist, int* obst, bool* toRaise, int s, int featureId) {
...
void MCMC3::lower(int city_size, std::list<std::pair<int, int> >& queue, int* dist, int* obst, bool* toRaise, int s, int featureId) {
	Point2D adj[] = {{-1, 0}, {1, 0}, {0, -1}, {0, 1}};
...
void MCMC3::updateDistanceMap(int city_size, std::list<std::pair<int, int> >& queue, int* zones, int* dist, int* obst, bool* toRaise) {
	while (!queue.empty()) {
...
void MCMC3::setStore(std::list<std::pair<int, int> >& queue, int* zones, int* dist, int* obst, bool* toRaise, int s, int featureId) {
	// put stores
...
void MCMC3::removeStore(std::list<std::pair<int, int> >& queue, int* zones, int* dist, int* obst, bool* toRaise, int s, int featureId) {
...
float MCMC3::min3(float distToStore, float distToAmusement, float distToFactory) {
 ...
 */
float MCMC3::computePriceIndex(std::vector<float>& feature) {
 ...
 */
void MCMC3::computeFeature(int city_size, int* zones, int* dist, int s, std::vector<float>& feature) {
...
/*
void MCMC3::computeRawFeature(int city_size, int* zones, int* dist, int s, std::vector<float> feature) {
	int cell_length = 10000 / city_size;
	...
	for (int i = 0; i < 7; ++i) {
		f[i] = distToFeature(feature[i]);
	}

	feature[7] = computePriceIndex(f); // 価格
}
*/

bool MCMC3::GreaterScore(const std::pair<float, int>& rLeft, const std::pair<float, int>& rRight) { return rLeft.first > rRight.first; }

/** 
 * ゾーンのスコアを計算する。
 */
float MCMC3::computeScore(int city_size, int* zones, int* dist) {
	int num_zones = 0;
 ...
 */
int MCMC3::check(int city_size, int* zones, int* dist) {
	int count = 0;
 ...
 * ゾーンプランを生成する。
 */
void MCMC3::generateZoningPlan(int city_size, int* zones, std::vector<float> zoneTypeDistribution) {
 ...
 */
void MCMC3::optimize(int city_size, int max_iterations, int* bestZone) {
 ...
 */
void MCMC3::optimize2(int city_size, int max_iterations, int* bestZone) {
	int* zone = (int*)malloc(sizeof(int) * city_size * city_size);
	int* dist = (int*)malloc(sizeof(int) * city_size * city_size * NUM_FEATURES);
	int* obst = (int*)malloc(sizeof(int) * city_size * city_size * NUM_FEATURES);
	bool* toRaise = (bool*)malloc(city_size * city_size * NUM_FEATURES);
