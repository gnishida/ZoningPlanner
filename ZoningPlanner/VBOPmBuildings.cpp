...
bool generateParcelBuildings(VBORenderManager& rendManager, Block &inBlock, Parcel &inParcel)
{
	...
	float bldgFootprintArea = inParcel.computeBuildableArea(inParcel.zone.parcel_setback_front, 
		inParcel.zone.parcel_setback_rear, 
	...
	if(!computeBuildingFootprintPolygon(
		inParcel.zone.building_max_frontage,
		inParcel.zone.building_max_depth,
		frontEdges, rearEdges, sideEdges,
		inParcel.parcelBuildableAreaContour.contour,
	...
	// stoties
	float heightDev = inParcel.zone.building_stories_deviation / 100.0f *
		(((float)qrand()/RAND_MAX)*2.0f-1.0f)* //LC::misctools::genRand(-1.0f,1.0f)*
		inParcel.zone.building_stories_mean;

	int bldgNumStories = (int)inParcel.zone.building_stories_mean + heightDev;
