...
void VBOTerrain::render(bool drawEditingCircle) {
	...
	if (editionMode && drawEditingCircle) {
		...
		// circle
		float radius = 100;//G::global().getFloat("2DterrainEditSize");
...
void VBOTerrain::excavate2(float u, float v, float height, float rad_ratio, cv::Mat& hiddenTerrain) {
	float x0 = u * _resolution;
	float y0 = v * _resolution;
	float rad = rad_ratio * _resolution;
	...
	for (int c = x0 - rad; c <= x0 + rad + 1; ++c) {
		...
		for (int r = y0 - rad; r <= y0 + rad + 1; ++r) {
			...
			float value = hiddenTerrain.at<float>(r, c);
			if (height == 72.0f) {
				if (value >= 72.0f) {
					layerData.at<float>(r, c) = height;
				}
				...
				if (fabs(value - height) < 1.0f || value >= 72.0f) {
					layerData.at<float>(r, c) = height;
				}
			}
		}
